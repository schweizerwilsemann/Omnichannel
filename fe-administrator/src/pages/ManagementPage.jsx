...
import MenuCategoriesPanel from "../components/categories/MenuCategoriesPanel.jsx";
...
const toPaginationState = (
  ...
  const totalItems =
    toNumber(pagination.totalItems) ?? Math.max(fallbackTotalItems, 0);
...
const ManagementPage = () => {
  const [activeKey, setActiveKey] = useState(MANAGEMENT_TABS.CATEGORIES);
  ...
  const loadMenuData = useCallback(
    async (overrides = {}) => {
      ...
      try {
        const response = await fetchMenuCatalog({
          page: nextPage,
        ...
        if (nextPage === 1) {
          let menuLibrary = Array.isArray(payload.items)
            ? [...payload.items]
            : [];
          ...
          if (totalPages > 1) {
            ...
            try {
              ...
              console.warn(
                "Unable to load additional menu items for combo selector",
                error,
              );
            }
          }
        ...
        toast.error(
          error.response?.data?.message || "Unable to load menu catalog",
        );
      } finally {
  ...
  useEffect(() => {
    if (
      activeKey === MANAGEMENT_TABS.CUSTOMERS &&
      !customersLoaded &&
      !loading.customers
    ) {
      loadCustomerData();
    ...
    if (
      activeKey === MANAGEMENT_TABS.TABLES &&
      !tablesLoaded &&
      !loading.tables
    ) {
      loadTableData();
  ...
  const handleCreateMenuItem = async (event) => {
    ...
    const payload = {
      ...
      prepTimeSeconds: menuForm.prepTimeSeconds
        ? Number.parseInt(menuForm.prepTimeSeconds, 10)
        : null,
    };
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Unable to create menu item",
      );
    } finally {
    ...
    try {
      ...
      setMenuData((prev) => ({
        ...
        items: prev.items.map((entry) =>
          entry.id === item.id
            ? updated || { ...entry, isAvailable: nextValue }
            : entry,
        ),
      }));
      toast.success(
      ...
      toast.error(
        error.response?.data?.message || "Unable to update menu item",
      );
    } finally {
      setMutations((prev) => {
  ...
  const handleCreateCombo = async (event) => {
    ...
    const payload = {
      ...
      prepTimeSeconds: comboForm.prepTimeSeconds
        ? Number.parseInt(comboForm.prepTimeSeconds, 10)
        : null,
    ...
    try {
      ...
      if (comboImageFile) {
        ...
        try {
          ...
          toast.error(
            error.response?.data?.message || "Unable to upload combo image",
          );
          return;
        } finally {
      ...
      toast.error(
        error.response?.data?.message || "Unable to create menu combo",
      );
    } finally {
  ...
  const handleToggleComboAvailability = async (combo) => {
    ...
    try {
      const response = await updateMenuCombo(combo.id, {
        isAvailable: nextValue,
      });
      const updated = response.data?.data;
      setMenuData((prev) => ({
      ...
      toast.success(
        nextValue
          ? "Combo is now visible to guests"
          : "Combo hidden from guests",
      );
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Unable to update menu combo",
      );
    } finally {
      setMutations((prev) => {
  ...
  const handleCreateCustomerMembership = async (event) => {
    ...
    const payload = {
      ...
      loyaltyPoints: Number.isNaN(loyaltyPointsValue)
        ? 0
        : Math.max(0, loyaltyPointsValue),
      discountBalanceCents: Number.isNaN(discountValue)
        ? 0
        : Math.max(0, Math.round(discountValue * 100)),
      customer: {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Unable to create customer membership",
      );
    } finally {
  ...
  const handleToggleMembershipStatus = async (membership) => {
    ...
    try {
      const response = await updateCustomerMembership(membership.id, {
        status: nextStatus,
      });
      const updated = response.data?.data;
      setCustomerData((prev) => ({
        ...
        memberships: prev.memberships.map((entry) =>
          entry.id === membership.id
            ? updated || { ...entry, status: nextStatus }
            : entry,
        ),
      }));
      toast.success(
        nextStatus === "MEMBER"
          ? "Customer promoted to member"
          : "Customer marked as guest",
      );
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Unable to update customer membership",
      );
    } finally {
      setMutations((prev) => {
  ...
  const menuLibrary = useMemo(
    () => menuData.menuLibrary || [],
    [menuData.menuLibrary],
  );
  ...
  const comboAvailableOptions = useMemo(
    () =>
      comboForm.restaurantId
        ? menuItemsByRestaurant.get(comboForm.restaurantId) || []
        : [],
    ...
    <MainLayout>
      <div className="d-flex flex-column gap-4">
        <div className="d-flex flex-wrap justify-content-between align-items-center gap-2">
          <div>
            ...
            <p className="text-muted mb-0">
              Manage menu items, loyal customers, and dining tables from one
              workspace.
            </p>
          </div>
        </div>
        ...
        <Tabs
          ...
          justify
        >
          <Tab eventKey={MANAGEMENT_TABS.CATEGORIES} title="Categories">
            <div className="pt-4">
              <MenuCategoriesPanel />
            </div>
          </Tab>

          <Tab eventKey={MANAGEMENT_TABS.MENU} title="Menu Items">
            <div className="pt-4 d-flex flex-column gap-4">
                  ...
                  >
                    ...
                    <Row className="g-3">
                      ...
                      <Col md={6}>
                        <Form.Group controlId="menu-image-file">
                          ...
                          />
                          <Form.Text muted>
                            Optional. Ảnh sẽ được tải lên MinIO và lưu đường dẫn
                            tự động.
                          </Form.Text>
                        </Form.Group>
                      </Col>
                    ...
                    {categoryOptions.length === 0 ? (
                      <Alert variant="warning" className="mb-0">
                        No menu categories found. Add categories through the API
                        or database before creating menu items.
                      </Alert>
                    ) : null}
                  </Form>
                </Card.Body>
                ...
                <Card.Body>
                  ...
                  >
                      ...
                      <Col md={4}>
                        <Form.Group controlId="combo-restaurant">
                          ...
                          <Form.Select
                            ...
                            required
                            disabled={
                              menuData.restaurants.length === 0 || saving.combos
                            }
                          >
                      ...
                      <Col md={3}>
                        ...
                        >
                          ...
                          <Form.Check
                            ...
                            name="isAvailable"
                            label={
                              comboForm.isAvailable ? "Available" : "Hidden"
                            }
                    ...
                    <div className="d-flex flex-column gap-2">
                          ...
                          >
                            ...
                            <Col xs={6} md={3}>
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleRemoveComboComponent(index)
                                }
                                disabled={
                                  saving.combos ||
                                  comboForm.components.length <= 1
                                }
                              >
                                Remove
                              </Button>
                      ...
                      comboAvailableOptions.length === 0 ? (
                        <Alert variant="warning" className="mb-0">
                          Add menu items for this restaurant before composing
                          combos.
                        </Alert>
                      ) : null}
                      {comboForm.restaurantId &&
              ...
              <Card className="shadow-sm">
                <Card.Body className="d-flex flex-column gap-3">
                  ...
                  {!loading.menu && menuCombos.length === 0 ? (
                    <Alert variant="light" className="mb-0">
                      No combos yet. Build your first combo above to bundle fan
                      favourites.
                    </Alert>
                  ) : null}
                  {menuCombos.length > 0 ? (
                          ...
                          {menuCombos.map((combo) => (
                            <tr key={combo.id}>
                              ...
                              <td className="text-center">
                                <Badge
                                  bg={
                                    combo.isAvailable ? "success" : "secondary"
                                  }
                                >
                              ...
                              <td className="text-end">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleToggleComboAvailability(combo)
                                  }
                                  disabled={Boolean(
              ...
              <Card className="shadow-sm">
                <Card.Body className="d-flex flex-column gap-3">
                  ...
                  {!loading.menu && menuItems.length === 0 ? (
                    <Alert variant="light" className="mb-0">
                      No menu items yet. Add your first dish using the form
                      above.
                    </Alert>
                  ) : null}
                  {menuItems.length > 0 ? (
                    <>
                            ...
                            {menuItems.map((item) => (
                              <tr key={item.id}>
                                ...
                                <td className="text-center">
                                  <Badge
                                    bg={
                                      item.isAvailable ? "success" : "secondary"
                                    }
                                  >
                                    {item.isAvailable
                                      ? "available"
                      ...
                      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 pt-3">
                        <div className="text-muted small">
                          {describeRange(menuPagination, menuItems.length)}
                        </div>
                        {menuPagination.totalPages > 1 ? (
                          <Pagination className="mb-0">
                            <Pagination.Prev
                              disabled={
                                !menuPagination.hasPrevious || loading.menu
                              }
                              onClick={() =>
                                handleMenuPageChange(menuPagination.page - 1)
                              }
                            />
          ...
          <Tab eventKey={MANAGEMENT_TABS.CUSTOMERS} title="Customers">
            <div className="pt-4 d-flex flex-column gap-4">
              ...
              <Card className="shadow-sm">
                <Card.Body>
                  <Form
                    onSubmit={handleCreateCustomerMembership}
                    className="d-flex flex-column gap-3"
                  >
                    <Row className="g-3">
                      <Col md={4}>
                    ...
                    <div>
                      <Button
                        type="submit"
                        disabled={
                          saving.customers || restaurantOptions.length === 0
                        }
                      >
                        {saving.customers ? (
                          <>
                    ...
                    {restaurantOptions.length === 0 ? (
                      <Alert variant="warning" className="mb-0">
                        No restaurants found for this account. Create
                        restaurants before adding memberships.
                      </Alert>
                    ) : null}
                  </Form>
                </Card.Body>
              ...
              <Card className="shadow-sm">
                <Card.Body className="d-flex flex-column gap-3">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div className="text-muted small">
                      {customerMemberships.length} records
                    </div>
                  </div>
                  ...
                  {!loading.customers && customerMemberships.length === 0 ? (
                    <Alert variant="light" className="mb-0">
                      No memberships found. Register a guest using the form
                              ...
                              <tr key={membership.id}>
                                <td>
                                  <div className="fw-semibold">
                                    {resolveCustomerName(membership.customer)}
                                  </div>
                                  <div className="text-muted small">
                                    #
                                    {resolveMembershipNumber(
                                      membership.customer,
                                    )}
                                  </div>
                                </td>
                                ...
                                <td className="text-end">
                                  {formatCurrency(
                                    membership.discountBalanceCents,
                                  )}
                                </td>
                                <td className="text-end">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleToggleMembershipStatus(membership)
                                    }
                                    disabled={Boolean(
                      ...
                      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 pt-3">
                        ...
                        {customerPagination.totalPages > 1 ? (
                          <Pagination className="mb-0">
                            <Pagination.Prev
                              disabled={
                                !customerPagination.hasPrevious ||
                                loading.customers
                              }
                              onClick={() =>
                            ...
                            <Pagination.Next
                              disabled={
                                !customerPagination.hasNext || loading.customers
                              }
                              onClick={() =>
                                handleCustomerPageChange(
                                  customerPagination.page + 1,
            ...
            <div className="pt-4 d-flex flex-column gap-4">
                  ...
                  >
                    ...
                    <div>
                      <Button
                        type="submit"
                        disabled={
                          saving.tables || restaurantOptions.length === 0
                        }
                      >
                        {saving.tables ? (
                          <>
                    ...
                    {restaurantOptions.length === 0 ? (
                      <Alert variant="warning" className="mb-0">
                        No restaurants found for this account. Create
                        restaurants before adding tables.
                      </Alert>
                    ) : null}
                  </Form>
                </Card.Body>
              ...
              <Card className="shadow-sm">
                <Card.Body className="d-flex flex-column gap-3">
                  ...
                  {!loading.tables && tableEntries.length === 0 ? (
                    <Alert variant="light" className="mb-0">
                      No tables registered yet. Use the form above to add your
                      first dining table.
                    </Alert>
                  ) : null}
                  {tableEntries.length > 0 ? (
                    <>
                            ...
                            {tableEntries.map((table) => (
                              <tr key={table.id}>
                                ...
                                <td>
                                  <Badge
                                    bg={
                                      TABLE_STATUS_VARIANTS[table.status] ||
                                      "secondary"
                                    }
                                  >
                                    {table.status
                      ...
                      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 pt-3">
                        <div className="text-muted small">
                          {describeRange(tablePagination, tableEntries.length)}
                        </div>
                        {tablePagination.totalPages > 1 ? (
                          <Pagination className="mb-0">
                            <Pagination.Prev
                              disabled={
                                !tablePagination.hasPrevious || loading.tables
                              }
                              onClick={() =>
                                handleTablePageChange(tablePagination.page - 1)
                              }
                            />
                            <Pagination.Item active>
                            ...
                            <Pagination.Next
                              disabled={
                                !tablePagination.hasNext || loading.tables
                              }
                              onClick={() =>
                                handleTablePageChange(tablePagination.page + 1)
                              }
          ...
          <Tab
            eventKey={MANAGEMENT_TABS.RECOMMENDATIONS}
